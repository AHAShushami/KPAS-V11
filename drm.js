const URL_DRP_HOSPITAL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSz79935IRGsGoV7cw9rsLQ62GIfUb0pLen4KjS-rJwzmGumggj97Sprb9582DW-A_jpG84bscwuX-w/pub?gid=1001329693&single=true&output=csv';
const URL_DRP_PKD = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSYOikPg-KWohovUcCkWgA6D1zolr6S0I8IHFc2c_C-Z4yfYh-KNfruiqXbRZs4OXuZcT6fT8As7ZDZ/pub?gid=248966659&single=true&output=csv';
let liveDRMData = [];
...
// Fetch both Hospital and PKD data
Promise.all([fetchCSV(URL_DRP_HOSPITAL), fetchCSV(URL_DRP_PKD)]).then(([hospData, pkdData]) => {
    const combined = [...hospData, ...pkdData];
    
    liveDRMData = combined.map(d => ({
        facility: d['Facility name'] || d.Facility || d.PKD || d['Facility Name'] || '',
        district: d.District || d.Daerah || '',
        teams: parseInt(d.Medical_Teams) || parseInt(d['Rapid Assessment Team (RAT)']) || parseInt(d['Medical Teams']) || 0,
        fourwd: parseInt(d.Four_WD) || parseInt(d['4WD']) || parseInt(d['4WD Vehicles']) || parseInt(d['4x4 Vehicles']) || 0,
        amb: parseInt(d.Ambulances) || parseInt(d['Type A Land Ambulance']) || parseInt(d['Type B Land Ambulance']) || 0,
        tents: parseInt(d['Disaster Tent']) || parseInt(d['Inflatable disaster tent']) || parseInt(d['Tents']) || 0,
        water: d.Water_Supply || d['Water tank storage - supply'] || 'Standby',
        raw: d // Store full raw data for the thematic scorecard
    })).filter(d => d.facility !== '');
    
    console.log("Loaded Combined DRM Data:", liveDRMData.length, "facilities");
