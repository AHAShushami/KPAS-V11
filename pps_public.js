...
function renderTable() {
    ...
    if (filteredData.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding: 2rem; color: var(--text-secondary);">No PPS matches your search criteria.</td></tr>';
        return;
    }
    ...
    filteredData.forEach(pps => {
        ...
        if (pps.statusFungsi.toLowerCase() === 'aktif') {
            opBadge = `<span class="badge-status badge-active">AKTIF</span>`;
        } else {
            opBadge = `<span class="badge-status badge-inactive">${pps.statusFungsi.toUpperCase()}</span>`;
        }

        // Verification Badge
        let verBadge = '';
        if (pps.statusLawatan === 'Telah Dilawati') {
            verBadge = `<span class="badge-status badge-verified">VERIFIED</span>`;
        } else {
            verBadge = `<span class="badge-status badge-pending">PENDING</span>`;
        }

        const coords = (pps.lat && pps.lng) ? `<a href="https://www.google.com/maps/search/?api=1&query=${pps.lat},${pps.lng}" target="_blank" style="color: #60a5fa; text-decoration: none;">🗺️ View Map</a>` : '<span style="color: var(--text-secondary);">N/A</span>';

        tr.innerHTML = `
            <td style="font-weight: 500;">${pps.district}</td>
            <td style="color: #fff;">${pps.name}</td>
            <td>${opBadge}</td>
            <td>${verBadge}</td>
            <td>${coords}</td>
        `;
